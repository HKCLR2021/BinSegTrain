...
from detectron2.layers import ShapeSpec, batched_nms_rotated, ROIAlignRotated, ROIAlign,cat, cross_entropy, nonzero_tuple
...
def fast_rcnn_inference_rotated(
    boxes, scores, offsets, image_shapes, score_thresh, nms_thresh, topk_per_image
):
    """
    ...
    result_per_image = [
        fast_rcnn_inference_single_image_rotated(
            boxes_per_image, scores_per_image, offsets_per_image, image_shape, score_thresh, nms_thresh, topk_per_image
        )
        for scores_per_image, boxes_per_image,offsets_per_image, image_shape in zip(scores, boxes, offsets, image_shapes)
    ]
    return [x[0] for x in result_per_image], [x[1] for x in result_per_image]

def fast_rcnn_inference_single_image_rotated(
    boxes, scores, offsets, image_shape, score_thresh, nms_thresh, topk_per_image
):
    """
    ...
    ##
    offsets = offsets.view(-1,num_bbox_reg_classes, 6) ############
    ...
    if num_bbox_reg_classes == 1:
        ...
        offsets = offsets[filter_inds[:,0], 0]
    else:
    ...
    boxes, scores, offsets, filter_inds = boxes[keep], scores[keep], offsets[keep], filter_inds[keep]
...
class OffRotFastRCNNOutputLayers(nn.Module):
    """
    ...
    def __init__(
        ...
        ##
        self.offset_pred = nn.Linear(input_size, num_bbox_reg_classes * 6)
        ##
    ...
    def forward(self, x):
        ...
        return scores, proposal_deltas, offsets
    ...
    def losses(self, predictions, proposals):
        ...
        """
        scores, proposal_deltas, offsets = predictions
        ...
        gt_offsets = cat([p.gt_offsets for p in proposals], dim=0) if len(proposals) else torch.empty(0)
        # parse box & offset regression outputs
        if len(proposals):
        ...
        losses = {
            "loss_cls": cross_entropy(scores, gt_classes, weight=torch.tensor([1., 1.]).cuda(), reduction="mean"),
            "loss_box_reg": self.box_reg_loss(
                proposal_boxes, gt_boxes, proposal_deltas, gt_classes
            ),
            "loss_offset_reg": self.offset_reg_loss(gt_offsets, offsets, gt_classes)
        }
    ...
    ##
    def offset_reg_loss(self, gt_offsets, pred_offsets, gt_classes):
        fg_inds = nonzero_tuple((gt_classes >= 0) & (gt_classes < self.num_classes))[0]

        fg_pred_offsets = pred_offsets.view(-1, self.num_classes, 6)[  ##########################
            fg_inds, gt_classes[fg_inds]
        ]

        if self.box_reg_loss_type == "smooth_l1":
            loss_box_reg = smooth_l1_loss(
                fg_pred_offsets, gt_offsets[fg_inds], self.smooth_l1_beta, reduction="sum"
            )
    ...
    def inference(self, predictions, proposals):
        ...
        ##
        offsets = self.predict_offsets(predictions,proposals)
    ...
    def predict_offsets(
        ...
        _, _, offsets = predictions
        num_inst_per_image = [len(p) for p in proposals]
        scaled_offsets = offsets * 200.0
        return scaled_offsets.split(num_inst_per_image, dim=0)
    ##
...
class MRROIHeads(StandardROIHeads):
    ...
    def _init_box_head(cls, cfg, input_shape):
        ...
        box_predictor = OffRotFastRCNNOutputLayers(cfg, box_head.output_shape)
        return {
    ...
    def label_and_sample_proposals(self, proposals, targets):
        ...
        for proposals_per_image, targets_per_image in zip(proposals, targets):
            ...
            if has_gt:
                ...
                proposals_per_image.gt_offsets = targets_per_image.gt_offsets[sampled_targets]
